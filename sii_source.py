...
from db_query_functions import exist_rut, query_get_data, query_post_data
from utils import create_format_response
...
class SIISource(DataSource):
    ...
    def get_profile(self):
        ...
        if not(exist_rut(self.rut)): 
            ...
            print("Rut existente")
            return self.bring_data_from_db()
    
    

    def bring_data_from_db(self):
        try:
            response_db = query_get_data(self.rut)
            
            if not(response_db):
                raise ValueError({'error': 'No se encontraron coincidencias o ocurrió un error al obtener datos'})
            
            activities = create_format_response(response_db)
            return activities
        
        except Exception as e:
            print({'error': f"Error al obtener o procesar datos: {str(e)}"})
            return None
    ...
    def scrap_sii_profile(self, req): 
        ...
        query_post_data(self.rut, name, activities)
        return {"nombre": name, "actividades": activities}
